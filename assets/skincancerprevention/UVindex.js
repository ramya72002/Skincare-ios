...
class UVindex extends Component {
  constructor(props) {
    ...
    this.subtitleImages = [
      require('./banners/sunsafety/image.png')
    ];
  }
  ...
  fetchUVindex = () => {
    ...
    fetch('https://backen-skin-care-app.vercel.app/uvindexdatat', {
      method: 'POST',
      headers: {
      ...
      .then((responseJson) => {
        this.setState({
          translatedUVindex: responseJson.tips,
          loading: false,
        });
      })
      .catch((error) => {
  ...
  render() {
    const { translatedUVindex, loading, error } = this.state;
    return (
        ...
        <View style={styles.scrollContainer}>
          {loading ? (
            ...
            <ScrollView contentContainerStyle={styles.scrollView}>
              {this.renderContent(translatedUVindex)}
              <UVTable />
            </ScrollView>
          )}
