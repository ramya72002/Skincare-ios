...
class Envdisorder extends Component {
  constructor(props) {
    super(props);
    const { preferredLanguage } = props.route.params;
    this.state = {
      translatedEnvdisorder: [],
    ...
    this.subtitleImages = [
      require('./banners/envdisorders/image1.png'),
      require('./banners/envdisorders/image221.png'),
      require('./banners/envdisorders/image331.png'),
      require('./banners/envdisorders/image4.png'),
      require('./banners/envdisorders/image2.png'),
      require('./banners/envdisorders/image44.png'),
      require('./banners/envdisorders/image55.png'),
      require('./banners/envdisorders/image66.png'),
      require('./banners/envdisorders/image77.png'),
      require('./banners/envdisorders/image88.png'),
      require('./banners/envdisorders/image5.png'),
      require('./banners/envdisorders/image6.png'),
    ];
  }

  componentDidMount() {
    this.fetchEnvdisorder();
  }
  ...
  fetchEnvdisorder = () => {
    ...
    fetch('https://backen-skin-care-app.vercel.app/envdisordert', {
      method: 'POST',
      headers: {
      ...
      .then((responseJson) => {
        this.setState({
          translatedEnvdisorder: responseJson.tips,
          loading: false,
        });
      })
      .catch((error) => {
  ...
  renderContent = (contentArray) => {
    ...
    content.push(
      <View key="additional-resource" style={styles.contentBlock}>
        ...
        <TouchableOpacity onPress={() => Linking.openURL('https://www.mayoclinic.org/diseases-conditions/vitamin-deficiency-anemia/symptoms-causes/syc-20355025')}>
          <Text style={styles.link}> &#8226;https://www.mayoclinic.org/diseases-conditions/vitamin-deficiency-anemia/symptoms-causes/syc-20355025</Text>
        </TouchableOpacity>
      </View>
    );
  ...
  render() {
    const { translatedEnvdisorder, loading, error } = this.state;
    return (
        ...
        <View style={styles.scrollContainer}>
          {loading ? (
            ...
            <ScrollView contentContainerStyle={styles.scrollView}>
              {this.renderContent(translatedEnvdisorder)}
            </ScrollView>
          )}
